...
- 新增默认启动的单进程 `enterprise-mcp` 网关：在 8010 同时发布会议室与 HR endpoint，不再按业务域增加容器；自动持久化 Ed25519 密钥并隔离私钥，API/worker 从 AgentRun 和钉钉 userId 逐次签发 audience-bound 短令牌，发现令牌禁止调用；会议确认以数据库唯一键处理并发，HR 永久 Token 仅由网关读取；镜像构建统一使用清华 Debian HTTPS 源，避免受限网络下无法安装系统依赖，并通过共享 uv BuildKit 缓存与源码/第三方依赖分层缩短重复构建时间。
- HR MCP 移除临时的人员总数与员工类别分布工具，改为打卡记录、考勤日明细、考勤汇总 3 个正式只读工具；查询 userid 只取自已验签的钉钉上下文并原样传递，模型仅提供日期范围，避免代查他人考勤。
- 统一模型可见的企业助手品牌文案：知识库、文档分析 Skill、子智能体工具及会议 MCP 描述由 Yuxi 改为 Xinbo，避免智能体自我介绍时暴露底层项目名。
- 修复钉钉机器人切换绑定智能体后复用旧智能体线程、导致消息投递返回“对话线程不存在”的问题；Channel 线程 ID 现在同时按用户、会话和智能体隔离。
- 钉钉 Channel 支持单进程多机器人账号：每个 RobotCode 在 API 固定绑定 Agent，并隔离 Stream、Card 凭证状态与连接重试；新增可配置的空闲会话自动重置，当前线程指针仅存 API 内存，旧单机器人配置继续兼容。
...
- 为 rag-agent 迁移补齐钉钉身份基础：用户身份按 corp_id + union_id 隔离，兼容历史 dingtalk uid 回填，并新增部门树、用户-部门关系、同步运行三张快照表；启动 schema 演进显式补齐已有 users 表字段和唯一索引。
- 登录页默认勾选用户协议与隐私协议同意项，仍支持取消勾选并阻止未同意时提交登录。
- 修复钉钉 H5 免登未接入前端的问题：补齐页面 JSAPI SDK 引入，兼容 PC 端免登 API，新增 `dd.requestAuthCode` 免登链路、非敏感配置接口，并避免 OIDC 回调页等待智能体初始化导致长时间转圈。
- 钉钉从 `/login` 入口打开时，路由守卫先完成免登并直接进入 `/agent`，不再展示普通账号密码登录页。
- 钉钉用户主动退出登录时保留登录页，不会被 H5 免登立即重新登录；重新从应用入口打开仍可自动免登。
- 按用户角色收敛前端入口：普通用户隐藏智能体、知识库/技能、模型选择、API Keys、环境变量和 Memory；所有角色隐藏文档中心与主题切换，移除 GitHub 推广入口，并为管理页面增加管理员路由保护。
...
- 修复钉钉等第三方导出的 docx 标题层级丢失：Docling 仅在样式定义含 `outlineLvl` 时才能输出标准 `##` Markdown 标题，而钉钉导出用自定义样式 ID（如 `dingding-heading2`）且缺 `outlineLvl`，导致标题全被解析为 `**加粗**` 纯文本。在 Docling 转换前自动遍历样式表，给含 “heading” 但缺 `outlineLvl` 的样式补上对应层级并写入临时文件，原文件不动；修正后 Docling 正常输出 `##`/`###` 标题，分块和知识图谱实体抽取均可利用标题层级。
